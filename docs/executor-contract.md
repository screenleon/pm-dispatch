...
| `cli-only` | `host-native` (e.g. claude-as-host), **or `cli-subprocess` with an explicit override** (e.g. codex) | No executor subagent self-writes a brief via a live host `Write` on this path: either the executor SELF-EXECUTES work-dir edits under the host harness (host-native), or its brief is pmctl-landed and it runs as an independent subprocess (codex). So a live hook must NOT gate it: the unified wrapper **no-ops when fired live** and enforces the brief-location policy only when driven by `pmctl guard check` (which sets `PM_GUARD_CHECK_CLI`). |
