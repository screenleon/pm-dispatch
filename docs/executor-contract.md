...
| Sandbox model | Isolation intent comes from the required handover `isolation_level:` field; the codex adapter translates it to codex-native `--sandbox`/`--approval`/`-c` flags (workspace-write default). | **Canonical (headless `claude --print` subprocess):** `isolation_level:` translates to a `--permission-mode`; the claude adapter tolerates any stray codex-native `--sandbox`/`--approval` CLI flag as a no-op. **Same-host fallback:** the main-thread execution surface governs edits directly. |
