...
- **Process kill** re-verifies PID/PGID/starttime/comm before signaling the process group (`SIGTERM`, then `SIGKILL` after `--grace` seconds). Signaling is allowed **only** when the recorded supervisor PID is still alive and matches the captured identity. If the leader is already gone, cancel **never** signals a still-live PGID (PID/PGID reuse risk) — it fails closed unless the group is already empty, in which case it may terminalize without a signal. Identity mismatch, missing identity, or non-isolated launch (`isolated=0`) also fail closed. Cancel refuses the caller's own process group and any group containing an ancestor of the cancel CLI.
- **Kill-before-claim**: when a live process must be signalled, the group is confirmed dead **before** the exclusive cancelled terminal claim is taken. A failed kill returns non-zero and does **not** mark the run cancelled. Once the claim is taken it is **never released** (the supervisor may already have observed it). The authenticated cancelled sentinel is then published so `wait` can resolve; if Run/Event/record writes are incomplete, cancel still exits non-zero while `wait` returns **130**.
...
Model B is the canonical dispatch topology: `pmctl dispatch run` lands the brief and the executor runs as an **independent subprocess** that consumes it (see DECISIONS.md 2026-06-15). Every Model B executor — codex, claude, opencode, grok, and future third-party adapters — MUST satisfy the following common contract. This is the baseline a new adapter is measured against; each requirement maps to an adapter self-check or a `doctor` check.
...
`--effort <low|medium|high>` is a `pmctl dispatch run` / `pmctl gate run` CLI flag, independent of `model` — it is not part of the `dispatch_handover_v1` metadata header. Precedence: `--effort` flag > the resolved model alias's own effort column (when valid) > global default `medium`. codex, claude, and grok wire it through natively (codex via `-c model_reasoning_effort=...`, claude via its own `--effort`, grok via `--reasoning-effort`); opencode has no equivalent and accepts the flag as a no-op with a warning. See `docs/dispatch-brief.md` §Reasoning effort and `runtime/lib/reasoning-effort.sh`.
...
| Reviewer pipeline trigger | `/pr-gate` dispatches the codex reviewer session via `pmctl dispatch run` and verifies `artifact_valid`, `subject_current`, and `policy_applicable` in-process (`pmctl gate verify <result> --cd <repo> --consumer embedded --json`). | `/pr-gate --executor claude` dispatches an **independent headless `claude --print` subprocess** (`adapters/claude/dispatch.sh`), symmetric with codex, and applies the same three-axis verification. The former `pr-gate-handover_v1` in-session fan-out was retired. |
...
`opencode` is a third shipped profile, not shown as a column above for brevity: `pmctl dispatch run --adapter opencode --brief-file <path>` invokes `adapters/opencode/dispatch.sh` (headless CLI subprocess, `runner_kind: cli-subprocess`, `write_guard_mode: cli-only`). It is the only adapter that accepts `isolation_level: none` (full machine access — opencode has no finer-grained sandbox); codex, claude, and grok reject that value. Model resolution uses `share/opencode-model-aliases.tsv` with a fallback chain across free-tier models. `--effort` is a non-goal for this adapter (no reasoning-effort equivalent in the opencode CLI); it is accepted as a no-op with a warning, mirroring the existing `--sandbox`/`--approval` no-op convention.

`grok` is a fourth shipped profile: `pmctl dispatch run --adapter grok --brief-file <path>` invokes `adapters/grok/dispatch.sh` (headless CLI subprocess, `runner_kind: cli-subprocess`, `write_guard_mode: cli-only`). Isolation is dual-layer — each `isolation_level` maps to both an OS-level `--sandbox` profile and a `--permission-mode` (see `adapters/grok/isolation-map.yaml`). Brief delivery uses `--prompt-file`; the JSONL stream is `--output-format streaming-json` with terminal event `end`. Model resolution uses `share/grok-model-aliases.tsv`. `--effort` maps to grok's native `--reasoning-effort`.

**Grok isolation narrowing (load-bearing):** Grok's OS sandbox has no CWD-only write profile — every write-capable profile also permits writes under `~/.grok` (session/config store) and temp dirs. For the grok adapter only, `workspace-write` / `workspace-network` / `sandboxed` therefore mean "Grok workspace/strict sandbox (CWD + `~/.grok` + temp)", not pure working-directory isolation. `read-only` still blocks project writes but still permits `~/.grok` + temp. `isolation_level: none` is rejected. pm-dispatch does not clean or roll back `~/.grok`; the adapter prints an observable stderr note when launching under a write-capable sandbox.

## Guard enforcement

Guard policy (what a role may write or run) is **executor-agnostic and lives in one place**: the guard hook scripts (`guard-pm-write.sh`, `guard-pm-bash.sh`, `guard-executor-write.sh`, `guard-reviewer-write.sh`), surfaced as a CLI via `pmctl guard check --event <pre-write|pre-bash|post-task> --role <pm|executor|reviewer> [--runtime <codex|claude>] --file/--command <val>`. Guard keys on the **role**, with the `--runtime` axis refining policy only where a role's policy genuinely differs by runtime; dispatch supplies the runtime via its `--adapter`. The `pre-bash` event registers no policy for `executor` or `reviewer` on any runtime (the codex-executor subagent that needed one was retired), so those cells fail-close. `pre-bash` for `pm` is the one runtime-specific exception: `runtime=codex` has a real policy (`guard-pm-bash.sh`, a curated denylist — a codex-hosted PM runs Bash directly), while every other runtime (claude's project-pm subagent never runs Bash itself) still fail-closes exactly like the other pre-bash cells. For `executor`, ONE unified `guard-executor-write.sh` covers every runtime: it derives the runtime from `agent_type` (`<runtime>-executor`) and reads that runtime's `write_guard_mode` from its adapter manifest. The role-level write policy is one shared rule, so adding a runtime needs no guard edit. The CLI synthesizes the canonical hook input and drives the same hook, so every host enforces the identical decision (deny → non-zero exit + reason).
...
| `cli-only` | `cli-subprocess` with an explicit override (all shipped adapters — codex, claude, opencode, and grok) | No executor subagent self-writes a brief via a live host `Write` on this path: the brief is pmctl-landed and the executor runs as an independent headless subprocess (codex, claude's `claude --print` route, opencode's CLI, and grok's CLI — all `cli-subprocess` overriding `write_guard_mode` to `cli-only`). So a live hook must NOT gate it: the unified wrapper **no-ops when fired live** and enforces the brief-location policy only when driven by `pmctl guard check` (which sets `PM_GUARD_CHECK_CLI`). |
...
| Non-Claude (e.g. codex-as-PM) | Native hook wiring is opt-in, not the default: `install.sh --enable-codex-command-guard` dispatches the manifest-owned `hosts/codex/bin/install.sh`, which splices a Codex `hooks.json` PreToolUse entry driving `guard-pm-bash.sh` — the same `pmctl guard check --role pm --runtime codex --event pre-bash` policy — automatically on every Bash call, matching Claude's automatic trigger. Without that opt-in, or on any other non-Claude host/event pm-dispatch has not wired a native hook for, the caller MUST call `pmctl guard check` explicitly and honor a non-zero exit as a deny. |

This asymmetry is inherent to the host's CLI capabilities — both paths evaluate the same policy. The **`--role`** selects the policy because each role has a different allow-list (pm pre-write → memory dir only; executor pre-write → `/tmp/brief-*.md` only); the **`--runtime`** axis refines it only where a role's policy genuinely differs by runtime. (`executor` and `reviewer` register no `pre-bash` policy on any runtime — the only executor one that did, codex's, was retired with the codex-executor subagent. `pm`/`pre-bash` is the exception: `runtime=codex` has a real policy, every other runtime still fail-closes.) Claude PreToolUse hooks may later shell to `pmctl guard check` to collapse to a single source, but today they remain the policy source the CLI composes.
...
| `3` | request recognized but **no policy registered** to evaluate it — `executor` + `runtime=claude` + `pre-bash` (claude headless subprocess governs its own Bash via `--permission-mode`; pm-dispatch registers no executor bash policy for claude), `executor` + any other runtime + `pre-bash` (no adapter needs one today), `pm` + any runtime OTHER than codex + `pre-bash` (claude's project-pm subagent never runs Bash itself), `reviewer` + `pre-bash` on any runtime, and the reserved-but-unimplemented `post-task` event. Distinct from `2` so a caller can tell "I cannot enforce this" apart from "this was denied". `pm/pre-bash` with `runtime=codex` is the one exception with a registered policy (`guard-pm-bash.sh`, a curated denylist — a codex-hosted PM runs Bash directly), so that specific cell returns `0`/`2`, not `3`. |
...
`runtime/lib/handover-validate.sh` accepts `executor: codex`, `executor: claude`, `executor: opencode`, and `executor: grok`; any other value is rejected. The `claude` adapter is `adapters/claude/dispatch.sh` (headless subprocess); the `opencode` adapter is `adapters/opencode/dispatch.sh` (headless subprocess, `isolation_level: none` only); the `grok` adapter is `adapters/grok/dispatch.sh` (headless subprocess, dual sandbox + permission-mode isolation). This document remains the upstream behavioral contract; future executors (e.g. other CLIs) should match the same input/output shape and add their entry to the executor enum + executor profiles table.
