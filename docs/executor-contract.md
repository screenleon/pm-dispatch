...
- **Process kill** re-verifies PID/PGID/starttime/comm before signaling the process group (`SIGTERM`, then `SIGKILL` after `--grace` seconds). Identity mismatch, PID reuse, missing identity, or a non-isolated launch (`setsid` unavailable → `isolated=0` in the identity file) **fail closed without signaling or terminalizing** (including when the recorded leader PID is already gone). Cancel also refuses to signal the caller's own process group or any group that contains an ancestor of the cancel CLI.
