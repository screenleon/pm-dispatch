...
5. **Fallback policy — no headless CLI → subagent path.** A runtime with **no** headless CLI cannot run Model B; it falls back to the subagent (Agent-spawn) route, which exists solely for that case and is gated by the live PreToolUse write hook. This fallback is **explicit, not the default** — every runtime with a CLI uses the subprocess route. (Note: codex's `Agent(codex-executor)` route, kept for ergonomic reasons in `agents/codex-executor.md` §When NOT to use, is **not** this class — it still dispatches the headless CLI via `pmctl dispatch run` and the main thread pre-writes the brief, so no subagent self-writes a brief and the live hook is never engaged. The live-hook write-guard branch survives only for a genuine no-CLI self-writing runtime, of which none ships today.)
...
|---|---|---|
| Invoker | PM writes the brief to a file and runs `pmctl dispatch run --adapter codex --brief-file <path>`, which validates + guards before invoking the `adapters/codex/dispatch.sh` adapter (codex CLI performs the execution step). | **Canonical:** `pmctl dispatch run --adapter claude --brief-file <path>` invokes the `adapters/claude/dispatch.sh` adapter, which runs headless `claude --print` as a CLI subprocess — host-independent, so codex-as-PM can drive it. **Same-host optimization:** when Claude IS the main thread, `agents/claude-executor.md` (Agent-spawn) runs the brief inline. |
| Sandbox model | codex-managed workspace-write semantics with explicit sandbox metadata in metadata header. | **Canonical (headless `claude --print` subprocess):** isolation translates to a `--permission-mode`; the codex-native `--sandbox`/`--approval` flags are accepted but ignored as no-ops (no codex sandbox metadata contract). **Same-host fallback:** the main-thread execution surface governs edits directly. |
| Write/Bash mechanism | codex CLI drives edits and command execution. | **Canonical:** the headless `claude --print` adapter subprocess drives edits and command execution via the Claude CLI (`runner_kind: cli-subprocess`); claude self-governs Bash via `--permission-mode`. **Same-host fallback only:** `agents/claude-executor.md` performs edits/checks directly on the main thread. |
| Reviewer pipeline trigger | `/pr-gate` dispatches the codex reviewer session via `pmctl dispatch run` and integrity-checks the result in-process. | `/pr-gate --executor claude` dispatches an **independent headless `claude --print` subprocess** (`adapters/claude/dispatch.sh`), symmetric with codex, and integrity-checks the result via `gate_result_verify` / `pmctl gate verify`. The former `pr-gate-handover_v1` in-session fan-out was retired (CC-383). |
...
| Suitable scope | Repo edits that are already in codex dispatch envelope. | Canonical: any dispatch where a headless `claude --print` subprocess is the executor (host-independent; codex-as-PM can drive it). Same-host main-thread editing is a secondary case (the `agents/claude-executor.md` fallback). |
...
| `cli-only` | `cli-subprocess` with an explicit override (both shipped adapters — codex and claude), **or `host-native`** (reserved for a genuine self-executing host, e.g. the same-host `Agent(claude-executor)` fallback route) | No executor subagent self-writes a brief via a live host `Write` on this path: either the brief is pmctl-landed and the executor runs as an independent headless subprocess (codex and claude's canonical `claude --print` route — both `cli-subprocess` overriding `write_guard_mode` to `cli-only`), or the executor SELF-EXECUTES work-dir edits under the host harness (`host-native`, the same-host fallback). So a live hook must NOT gate it: the unified wrapper **no-ops when fired live** and enforces the brief-location policy only when driven by `pmctl guard check` (which sets `PM_GUARD_CHECK_CLI`). |
...
**On the two dispatch entrypoints (single policy, not split-brain):** `pmctl dispatch run` is the **sole routine** codex path and the policy surface — it runs `brief-validate` + `pmctl guard check` before invoking the adapter, and no executor subagent ever holds brief-write authority on this route. The codex adapter (`adapters/codex/dispatch.sh`) is also directly callable as the `Agent(codex-executor)` **fallback** route; there the brief file is pre-written by the main thread (codex-executor has no `Write` tool), and because codex's `write_guard_mode` is `cli-only` the live PreToolUse hook **no-ops** — the brief-location policy is enforced via `pmctl guard check`, not a live hook. So neither codex entrypoint relies on the live write-hook; both share one policy and the directly-callable adapter is the low-level primitive, not an unguarded bypass. A non-Claude host that calls the adapter directly (outside both `pmctl dispatch run` and a PreToolUse-capable agent) is responsible for calling `pmctl guard check` itself, exactly as the non-Claude row above requires.
