...
1. **Rule A `/tmp/<slug>/` shape** — accept loose `[a-z][!/]*` (any lowercase-prefixed single segment) or require `-content` suffix (`[a-z][!/]*-content`)? Recommendation: loose, because live traffic uses both `cc060-` style and `cc249-prb2-content` style, and a stricter rule would just force PM to rename directories.
2. **Memory-private root configurability** — hard-code `$HOME/github/memory-private` (matches `[[reference_memory_private_repo]]`) or read from `CLAUDE_MEMORY_PRIVATE_ROOT` env? Recommendation: env var with default, for portability to other users.
3. **`docs/spikes/` filename allowlist scope** — confirm `CC-NNN*.md | *-scope.md | *-rfc.md` is sufficient. Should `*-design.md`, `*-proposal.md` be added pre-emptively, or wait for audit evidence?
4. **Bypass mechanism** — keep single `CLAUDE_HOOK_PM_GUARD=off` (recommendation), or split into per-rule bypasses? Audit shows current bypass is only used in tests, suggesting single bypass is fine.
5. **Spike scope vs spike output** — should the rule distinguish a *pre-impl scope doc* (PM authorship) from a *spike report with verdict* (main-thread validation territory per `[[feedback_spike_validation_mandatory]]`)? Today both live in `docs/spikes/`. If they should be split, recommend: `docs/spikes/scope/*.md` for PM, `docs/spikes/report/*.md` for main thread. (This is a separate restructuring; out of scope for the hook policy.)
6. **Rollout** — land hook change + tests in one PR, or split (a) test additions that document desired-but-not-yet-implemented behavior as `should_run` skips, then (b) implementation that flips them on, for cleaner reviewability? Recommendation: single PR (test + code together), since the change is small enough to review atomically.
