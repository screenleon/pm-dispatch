...
Use `pmctl dispatch wait <run_id> --cd <work_dir> [--timeout <secs>]` to reattach and resolve the terminal outcome. `--cd` is mandatory; timeout exits 124. The authoritative completion signal is the supervisor sentinel written to `/tmp` (not the in-workspace `.dispatch-results/<run_id>.md` record, which is a human-readable observability artifact). The sentinel path includes a nonce not stored in the workspace run-spec, so the executor cannot forge the correct path; the durable record is read only for its human-readable summary if available.
