...
Use `pmctl dispatch wait <run_id> --cd <work_dir> [--timeout <secs>]` to reattach and resolve the terminal outcome from the durable `.dispatch-results/<run_id>.md` record. `--cd` is mandatory; timeout exits 124; absent record is treated as non-terminal.
