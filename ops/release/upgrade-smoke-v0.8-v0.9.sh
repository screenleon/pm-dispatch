#!/usr/bin/env bash
# CC-501 one-shot v0.8.0 -> v0.9 candidate upgrade acceptance smoke.
set -euo pipefail
export LC_ALL=C.UTF-8

usage() {
  cat <<'EOF'
Usage: upgrade-smoke-v0.8-v0.9.sh --baseline-dir PATH --candidate-dir PATH [options]

Required paths must be independent git checkouts/worktrees.

Options:
  --baseline-dir PATH  checkout at v0.8.0
  --candidate-dir PATH checkout containing the v0.9 candidate
  --artifact-dir PATH  evidence destination (default: .gate-results/cc501-<UTC>)
...
EXPECTED_BASELINE_SHA="$(git -C "$BASELINE_DIR" rev-list -n 1 v0.8.0 2>/dev/null || true)"
[[ -n "$EXPECTED_BASELINE_SHA" && "$BASELINE_SHA" == "$EXPECTED_BASELINE_SHA" ]] || {
  printf 'upgrade-smoke: baseline is not tag v0.8.0 (%s)\n' "$BASELINE_SHA" >&2
  exit 2
}

if [[ -z "$ARTIFACT_DIR" ]]; then
  ARTIFACT_DIR="$CANDIDATE_DIR/.gate-results/cc501-$(date -u +%Y%m%dT%H%M%SZ)"
fi
mkdir -p "$ARTIFACT_DIR"
ARTIFACT_DIR="$(cd "$ARTIFACT_DIR" && pwd -P)"
SANDBOX="$(mktemp -d /tmp/cc501-upgrade.XXXXXX)"
...
OLD_OPENCODE_OWNER="$SANDBOX/v0.8-opencode-managed-fixture"
...
write_fixtures() {
  ...
  # v0.8.0 predates the OpenCode writer. Establish the representative
  # receipt-owned old-checkout state required by CC-501 with the compatibility
  # writer, while keeping the release baseline itself at the exact v0.8.0 tag.
  mkdir -p "$OLD_OPENCODE_OWNER"
  cp -R "$CANDIDATE_DIR/runtime" "$CANDIDATE_DIR/hosts" "$CANDIDATE_DIR/cli" "$OLD_OPENCODE_OWNER/"
  bash "$OLD_OPENCODE_OWNER/hosts/opencode/bin/install.sh" --repo-root "$OLD_OPENCODE_OWNER" \
    > "$ARTIFACT_DIR/baseline-opencode-fixture.log" 2>&1
}
...
run_stage candidate-install bash "$CANDIDATE_DIR/install.sh" --profile minimal --enable-host codex --enable-host opencode
...
if { grep -RFn -- "$BASELINE_DIR" "$CLAUDE_ROOT/settings.json" "$CODEX_ROOT/hooks.json" \
      ...
      || grep -RFn -- "$OLD_OPENCODE_OWNER" "$CLAUDE_ROOT/settings.json" "$CODEX_ROOT/hooks.json" \
      "$XDG_ROOT/opencode/opencode.json" "$XDG_ROOT/opencode/commands" "$XDG_ROOT/opencode/tools"; \
...
run_stage candidate-dry-run bash "$CANDIDATE_DIR/install.sh" --dry-run --profile minimal --enable-host codex --enable-host opencode
...
jq -n \
  --arg ticket CC-501 --arg baseline_ref v0.8.0 --arg baseline_sha "$BASELINE_SHA" \
...
printf 'CC-501 upgrade smoke: GO\n'
printf 'baseline:  %s\n' "$BASELINE_SHA"
printf 'candidate: %s\n' "$CANDIDATE_SHA"
printf 'artifact:  %s\n' "$ARTIFACT_DIR"
