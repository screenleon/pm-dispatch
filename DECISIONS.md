...
**Decision**: Re-anchor v0.5.0 Phase 2 from scattered capabilities (CC-234 / CC-235 / knowledge index) to **memory read + write both usable**, success criterion = "a memory written this week is reachable next week, and the reflex is query-before-grep instead of grep-first". Split into a read half (CC-354, new) and a write half (CC-234, re-scoped). Pull the **anchored-TOC slice** of CC-340 forward into CC-354 (per-section chunking of in-repo knowledge docs via a pluggable per-format chunker); CC-340 narrows to the heavy remainder (embeddings, full-text ranking, episodic chunks) and stays v0.6.0.
