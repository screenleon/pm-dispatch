...
A local-first control plane for planning, dispatching, verifying, and shipping work across AI coding agents. pm-dispatch runs Claude, Codex, and OpenCode behind one host-independent runtime — shared policy, state, and evidence — instead of three incompatible tool-specific workflows, plus Grok as a dispatched executor (its own PM-host role is an early, batch-only MVP with no guard wiring yet).
...
- [Adapter manifest contract](docs/adapter-contract.md) — canonical dispatch entrypoint, safe path resolution, and schema v1 migration
...
Executors (codex, claude, opencode, grok) are **not** subagents — the main thread dispatches each as an independent CLI subprocess via `pmctl dispatch run --adapter <name>`, then verifies via `git diff` and `dispatch-post-verify.sh`.
...
- **cli/pmctl** — Runtime CLI spine. `install.sh` symlinks it into `${PMCTL_BIN_DIR:-$HOME/.local/bin}` on Linux/WSL2. Key sub-commands: `pmctl pm prepare` / `pmctl pm run` (batch-only PM coordinator for non-Claude hosts); `pmctl dispatch run --adapter <name> --brief-file <path>` (preferred manifest-driven dispatch path; built-ins: codex, claude, opencode, grok); `pmctl task create/show/list/update/claim/dispatch/status/review` (task CRUD + lifecycle; see [`docs/pmctl-task.md`](docs/pmctl-task.md)); `pmctl decision add`; `pmctl trace tail` (reads `events.jsonl` + archives with `--kind/--task/--since/--until/--json` filters); `pmctl context index/update/query/pack/reuse-scan` (repo index + prior-art scan; see [`docs/context-retrieval.md`](docs/context-retrieval.md)); `pmctl validate brief`; `pmctl gate run`; `pmctl guard check --role <pm|executor|reviewer> --runtime <codex|claude>`; `pmctl safe bash`; `pmctl adapter generate <name>` (scaffolds `adapters/<name>/adapter.yaml` and support files). `adapter.yaml` is the authored source of truth; the [Adapter manifest contract](docs/adapter-contract.md) defines its runtime-authoritative `dispatch_entrypoint` and bounded schema v1 migration. The manifest itself must stay out of `generated_files`; regenerate only the files listed there.
...
- **tools/lint/lint-shellcheck.sh** — CI/local ShellCheck entrypoint. `.shellcheck-version` is the exact shared toolchain authority; `tools/lint/bootstrap-shellcheck.sh` installs its checksum-verified official asset into the tool cache. The linter resolves that pinned binary offline — `PATH` when it matches the pin, otherwise the already-populated cache — so a sandbox carrying a different ambient `shellcheck` still scans with the pinned one; it fails before scanning, naming both probes, only when neither can supply the pin. Resolution never downloads. A cached binary is authenticated by the `binary_sha256` its manifest row records, not by the version it reports about itself, so a replaced cache entry is rejected before it is ever executed. The linter discovers `.sh` files from `shellcheck-domains.tsv` across `runtime/`, `tests/`, `tools/`, `ops/`, `hosts/`, and bounded `scripts/` compatibility shims; code-scoped canonical-path exceptions require a reason in `shellcheck-ignores.tsv`. It defaults to two CPU-bound workers; set `PM_DISPATCH_SHELLCHECK_JOBS` to an explicit value from 1 through 8 to tune concurrency.
...
**Audit log:** every hook firing that targets the matched subagent appends one line to `$PM_GUARD_LOG_DIR/hooks.log`; without that explicit binding it uses the product-owned state log root (`$PM_DISPATCH_STATE_ROOT/logs`, then XDG state). No-ops for other agents are not logged. Format: `<ISO8601> <hook-name> agent=<type> tool=<name> decision=<allow|deny|bypass> reason=<...> target=<path-or-cmd>`. `reason` and `target` are `printf %q`-escaped so the log is safely re-parseable. The `PM_GUARD_LOG_DIR` env var lets the test suite redirect to a sandbox dir without polluting the live log.
...
- **Hooks enforce hard rules; prose alone leaks.** CLAUDE.md compliance for "never do X" rules sits around 70% in the public research, so structural enforcement matters for invariants. The live `PreToolUse` hook in `~/.claude/settings.json` is `guard-pm-write.sh`: project-pm cannot edit canonical memory directly, and may write only the narrow task-brief/spike handoff paths. Canonical memory mutations go through the `pmctl` writer surfaces. Executor write policy is enforced by the unified `guard-executor-write.sh` via `pmctl guard check` (cli-only — the brief is authored by trusted main-thread code, so no live write hook is needed). All guards no-op for the main thread and other subagents.
  ...
  - **Bypass**: `PM_GUARD_PM_WRITE=off` / `PM_GUARD_<RUNTIME>_WRITE=off` (e.g. `PM_GUARD_CODEX_WRITE=off`). Each bypass appends a line to the product-owned guard log.
...
- **Auto-injection hooks are a recurring per-turn tax, not a one-time load.** `guard-inject-memory.sh` and the context prompt-scan both fire on every `UserPromptSubmit`, so their cost is paid every message, not once per session, and neither block benefits from prompt caching (each is re-ranked against the current prompt). Rough sizes: the memory-index hook runs ~600–1300 tokens/turn against a 3000-byte/20-entry budget (`lib/memory.sh`) that silently drops most cards once a store outgrows the window — see [memory-system.md § Per-prompt token cost](docs/memory-system.md#per-prompt-token-cost). The context prompt-scan is smaller and self-limiting, ~100–400 tokens on a hit and fully silent on a miss, with a real off switch (`PM_DISPATCH_DISABLE_PROMPT_CONTEXT=1`) — see [context-retrieval.md § Prompt auto-scan](docs/context-retrieval.md#prompt-auto-scan-deterministic-retrieval-at-prompt-time). The memory hook has no equivalent env toggle; disabling it means unwiring it from `~/.claude/settings.json`. Trade-off in one line: flat per-turn token cost in exchange for structural continuity (prose-only "remember this" rules leak; a hook does not) and usage-ranked recall as the store grows.
