...
- **`adapters/codex/dispatch.sh`** — dispatch now pins pm-dispatch's own default model (`DEFAULT_DISPATCH_MODEL=gpt-5.5`, fallback `gpt-5.4`) instead of letting omitted `--model` inherit the host's `~/.codex/config.toml` default. Previously, hosts whose interactive codex default was `gpt-5.3-codex-spark` silently ran **pr-gate and `/pm` dispatch on the spark model** (lower context ceiling, separate usage pool) — wrong for analysis-heavy work. The reserved `dispatch.default_model` config key is now wired (precedence: `--model` flag > config > built-in gpt-5.5); `codex-spark` stays opt-in. `share/model-aliases.tsv` gains `gpt-5.5`/`gpt-5.4` rows and a corrected default-model note (CC-292).
