...
- **`scripts/lib/pmctl-config.sh`** — shared config loader for `dispatch.default_timeout` + `dispatch.default_model` (CC-293 partial, PR #216). Centralizes `~/.pm-dispatch/config` key parsing; removes ~50 LOC of duplicated `_load_pm_config`/`_load_config_timeout` functions from `adapters/codex/dispatch.sh` and `adapters/claude/dispatch.sh`. Adapters call `pm_config_load` guarded by `type -t` so test environments that bypass self-snapshot degrade silently rather than hitting command-not-found (exit 127).
