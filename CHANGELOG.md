...
- **`install.sh` / `uninstall.sh` / `scripts/install-hooks.sh`** — honor an explicit `CLAUDE_HOME` env override so install/uninstall can target an alternate directory (sandbox / rehearsal) without overriding the whole `$HOME`. Previously `CLAUDE_HOME` was hardcoded to `$HOME/.claude`, and bare `$HOME/.claude` references (`.pm` dest, the install manifest, and the hook `settings.json` written by `install-hooks.sh`) bypassed it — so there was no safe way to test an install without touching the real config. All destinations now derive from `CLAUDE_HOME` (passed per-call to `install-hooks.sh`, not exported, so it does not leak into the `--verify` preflight's nested test installs). New `test-install.sh` cases assert an override redirects both install and uninstall and leaves `$HOME/.claude` untouched (CC-294).
