...
6. On wait exit 124 (sentinel not yet written within the timeout), the supervisor may still be running — do **not** re-dispatch. Retry `pmctl dispatch wait <run_id>` exactly once with the same `run_id`. If it times out again, stop and surface the supervisor log at `$(pmctl artifacts show <run_id> --cd <working_dir>)` for the user.
