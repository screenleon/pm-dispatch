...
Invoke `project-pm` via Agent with `model: "sonnet"` (per `docs/model-tier-policy.md` — always specify the model param explicitly). Brief with: request ($ARGUMENTS), current working directory, and relevant prior-turn context the subagent won't otherwise see.
