...
| CC-225 | ⏸ deferred | **[claude-executor result observability]** `claude-executor` task output 寫入 session-scoped `/tmp/` 路徑，不進 REPO、不可跨 session 回溯，且無法 git diff 追蹤執行歷史。設計目標：主線程在 claude-executor 完成後把 brief 路徑、result 摘要、exit status 寫入 REPO 固定目錄（格式與 `.gate-results/` 一致），作為 CC-211 / CC-216 MCP 架構抽離的前提。sub-concern of CC-211. | arch/observability | 2026-05-22 | — | P3 | design |
| CC-226 | ⏸ deferred | **[shellcheck coverage for doctor.sh + test-doctor.sh]** `doctor.sh` and `test-doctor.sh` are on the shellcheck ignore list in `.github/workflows/lint.yml` because `shellcheck` was not available at authoring time. Remove both files from the ignore list and fix all shellcheck warnings so the highest-risk shell constructs in the doctor module are covered by the CI linter. Raised by critic [low] in gate-20260522-145444. | ops/DX | 2026-05-22 | — | P3 | oss |
