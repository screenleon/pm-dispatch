...
| CC-222 | ⏸ deferred | **[pr-gate diff classifier]** Before reviewer agents run, execute a lightweight pre-phase agent that reads the diff and auto-classifies which review dimensions are relevant (code/test/ops/security/docs). Use the result to skip irrelevant reviewers and auto-determine tier — replacing manual `--scope` flag usage. Expected benefit: zero-waste reviewer sessions on docs-only or ops-only PRs; automatic security-reviewer trigger on auth/input-handling changes. | gate/DX | 2026-05-21 | — | P3 | design |
...
## CC-222 — pr-gate diff classifier（deferred）

**Problem**: All reviewer agents currently receive the full diff regardless of which review dimensions are actually relevant. A docs-only change triggers the same reviewer set as a security-sensitive auth change. The manual `--scope` flag exists but requires user judgment on every invocation.
**Why**: Wasted reviewer sessions increase token cost and latency. Security-reviewer and risk-reviewer are only run when explicitly requested via `--scope`, which means sensitive paths may be missed when users forget to pass the flag.
**Requirement**: Implement a lightweight pre-phase agent that reads the diff and classifies which review dimensions are relevant (code/test/ops/security/docs). The classifier outputs a recommended reviewer set and tier. Integrate into `pr-gate.sh` as an optional pre-pass before reviewer dispatch, with `--no-classify` to opt out. Expected benefit: zero-waste reviewer sessions on docs-only or ops-only PRs; automatic security-reviewer trigger on auth/input-handling changes.
**Priority**: P3. Implement after CC-206 (gate lifecycle hooks) — both touch the pre-dispatch phase of pr-gate.sh.
