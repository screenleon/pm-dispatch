...
**Problem**: `claude-executor` 走 Agent tool subagent + Claude 內建 Read/Edit/Write 直接操作，`codex-executor` 走 `codex-dispatch.sh` → Codex CLI。兩條路徑設計上不一致：`Agent(subagent_type="claude-executor")` 讓 Claude 在自己內部扮演另一個 Claude，不是對外部 CLI 的呼叫，繞過了外部 dispatch 的安全邊界與 permission 機制。

**Requirement**: 兩者都走 `pmctl dispatch run --adapter codex/claude --brief-file /tmp/brief.md`，分別呼叫外部 CLI（Codex CLI / `claude --print`）。主線程一律用 bash 呼叫外部 dispatch，不使用 Agent tool 執行 implementation 任務。

**Scope**:
- `agents/claude-executor.md` — 角色定義改寫：執行方式改為透過 `adapters/claude/dispatch.sh` 呼叫 `claude --print`
- `agents/codex-executor.md` — dispatch 路徑改為 `pmctl dispatch run --adapter codex`
- 主線程 dispatch 呼叫點 — 移除 `Agent(subagent_type="claude-executor")` 執行任務的用法
- `memory/feedback_codex_529_fallback.md` — 已更新（2026-06-01）：permission denied ≠ 529，前者要診斷根因
