...
新增 `pr-gate.sh --head <ref>`：省略時維持現有行為（working tree / 當前分支 fallback）；指定時走獨立分支直接 `git diff "$BASE"..."$HEAD_REF"`，不觸碰 working tree 或 dirty-preflight 邏輯，因此與 `--allow-dirty`（其存在目的是把 working tree 折入 scope）明確互斥並拒絕（exit 2）。Reviewer brief context block 在 `--head` 生效時額外顯示 `Head: <ref>` 一行。`pmctl gate run` 兩條路徑（foreground exec 與 `--lifecycle detached` 透過 `gate-supervisor.sh` 的 `--` passthrough）皆無需改動即可轉發 `--head`，因為都是把未知旗標原樣傳給 `pr-gate.sh`。`--help` usage block 與 unknown-arg 提示同步補上。新增 3 個測試（固定 ref diff 正確性、無效 head ref 報錯、`--head`+`--allow-dirty` 衝突拒絕）於 `scripts/test-pr-gate.sh`，122 項全數通過；`test-gate-lifecycle.sh`/`test-pmctl-gate.sh` 無回歸。
