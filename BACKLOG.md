...
| CC-375 | 🔵 active | **[install: hook 接線由 manifest 能力旗標衍生（CC-385a 縮範圍）]** `install-hooks.sh`/`uninstall-hooks.sh`/`doctor.sh` 對「哪些 hook 該接進 settings.json」目前逐 executor 寫死。**Rescoped after CC-385a spike (2026-06-15)**：codex 已宣告 `write_guard_mode: cli-only`，所有 adapter 現為 cli-only，不再需要 manifest-drive executor live write-guard 接線。縮範圍為：(a) manifest-driven **bash-guard** 接線（`needs_bash_guard=true` + `adapters/<name>/bash-guard.sh`）；(b) 從 existing installs **prune `hook-executor-write-guard.sh`** PreToolUse entry（live hook 成 no-op，退場以保持三方一致）；(c) doctor 一致性。釘死 install/uninstall/doctor 三方一致（呼應 [[CC-368]]）。相依 [[CC-374]]、[[CC-385a]]。umbrella [[CC-333]]。 | arch/install | 2026-06-13 | — | P3 | hygiene |
