...
**Problem**: `install.sh preflight` (lines 108-185) runs all test suites when `--verify` is passed, and any environment-specific failure (missing bin, wrong PATH) aborts the entire install. Tests are hosted *inside* install.sh rather than in a standalone runner.
...
- (b) Replace install.sh preflight block (lines 128-195) with a single `bash "$REPO_ROOT/scripts/run-all-tests.sh"` call
