...
| CC-104g | 🔵 active | **[Windows dogfood r1 fixes]** `scripts/lib/portable.sh` test failures: (a) `realpath_m_symlink_resolves` test setup used `ln -s` which falls back to copy on Git Bash → skip when precondition fails; (b) `mkdir_lock_contention_timeout` race in test using `sleep 0.1` → use marker-file synchronization; (c) `detect_platform_host_linux` asserted `==linux` regardless of host → renamed `host_native`, dynamic expected | ops/test | 2026-05-17 | (this branch) |
| CC-104h | 🔵 active | **[Windows dogfood r1 fixes]** `handover-validate.sh` brief_file validator only accepted `/tmp/brief-*.md`; now also accepts paths under `$TMPDIR / $TEMP / $TMP` (POSIX or MSYS forward-slash form). Windows backslash paths still rejected at metadata-metachar level — document as limitation (Windows briefs must use forward-slash form) | ops/validator | 2026-05-17 | (this branch) |
| CC-104i | 🔵 active | **[Windows dogfood r1 fixes]** `.gitattributes` forces LF for `*.sh` / `*.md` / `*.yml` / etc. Prevents CRLF drift on Windows under `core.autocrlf=true` (Git for Windows default) | ops/repo | 2026-05-17 | (this branch) |
