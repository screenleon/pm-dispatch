...
| CC-489 | 🔵 active | `scripts/` domain ownership 重整：Phase 0–3 已完成 inventory、module ABI、host resolver 與 OpenCode pilot migration；下一階段為 Codex host migration，之後才是 Claude 與 shared runtime/ops/test domains | arch | 2026-07-14 | feedback:2026-07-16 | P2 | design |
...
**Phase 3 completed (2026-07-16)**: OpenCode install/uninstall/doctor 已搬至 `hosts/opencode/{bin,lib}` 並由 `host.yaml` 發現；shared doctor loader 改為依 manifest `doctor_module` 載入，舊 install/uninstall 入口只保留 stderr deprecation 的 thin shim，舊 internal doctor path 已移除。搬遷過程暴露的 minimal-`PATH` 問題已在 shared manifest reader 以純 Bash path/scalar parsing 收旂，changed-path planner 也將 shared manifest 變更對應到三 host write/parity/doctor suites。refactor/reuse 稽核將 doctor、host-write、path resolver 三處新增的 module path 解析與 fail-closed 驗證收斂至 `host_manifest_module_path`；legacy shims 則刻意保持 self-contained，避免相容入口新增脆弱依賴。path inventory 現為 173 個 `scripts/` files，variable graph 為 62 declarations/317 refs；13 個 direct-impact suites 全綠（含 OpenCode 15、parity 8、doctor 64、host manifest 91、Codex host-write 45 cases）。
