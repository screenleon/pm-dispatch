...
| CC-330 | 🔵 active | **[fix: state_store_init — propagate layout mkdir failure loud]** `state_store_init` 的 project layout 建立（`mkdir -p tasks/ reviews/ …`）仍用 `2>/dev/null || true` 靜默吞掉失敗；與 VERSION 驗證的 fail-loud 語意不一致，存在 VERSION=1 但 `proj_dir` 目錄建立失敗時 `state_store_init` 仍回 0 的缺口。見 CC-311/312 PR #230 gate advisory（critic + arch-reviewer low）。 | arch | 2026-06-05 | — | P3 | hygiene |
