...
| CC-385 | 🟢 someday | **[spike: dispatch 模型統一 — brief 由可信代碼落地、executor 一律獨立子程序消費]** 退場「子代理自寫 brief」路（codex 現況、需 live hook），改為 `pmctl` 落地 brief → executor 當獨立子程序帶入消費（claude 經 [[CC-383]] 已是此模型、`pmctl dispatch run` 既有此路）。動機：live-hook write-guard 的存在**唯一原因**就是 codex-executor 子代理自寫 brief；統一後 `write_guard_mode` 可一律 cli-only、live-hook 機制可退役、guard 變 runtime-agnostic（推進 [[CC-333]] layer 2）。User 框定（2026-06-14）：(1) AUTH 假設預先登入好，故 Model A 的憑證繼承優勢不計；(2) 主線程已了解專案，分發子代理再重讀=重複 token 消耗，Model B 去掉中間人、brief 帶入消化過的 context（關 [[CC-366]]）。待解：D1 誰寫 brief（不可 PM 經 Write，撞 pm-write-guard → 由 pmctl 落地）、D2 context-pack 進 brief、D3 executor 獨立 auth 前提、D4 是否全退子代理路（牴觸 [[dispatch-route-primary]]）、D5 guard collapse 收尾順序保持 fail-closed。**[[CC-374]] 是地基**（write_guard_mode 已 manifest 化，翻 codex→cli-only 僅一行）。Scope: `docs/spikes/CC-385-dispatch-model-unification-scope.md`。排在真 adapter [[CC-376]]/[[CC-377]] 之後。 | arch/portability | 2026-06-14 | — | P2 | spike |
