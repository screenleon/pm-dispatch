...
| CC-209 | 🟢 someday | **[context-enrichment spike: context-pack source evaluation]** Evaluate external OSS projects as candidate sources for the future context-pack (CC-232) + context-enricher (CC-237). Partial spike result `docs/spikes/CC-209.md` (2026-05-24) covers Memori + agentmemory + portability lens with pilot walkthrough; codegraph survey deferred (still owed per original scope: install + query + token/accuracy benchmark vs rg/git baseline). Spike is inventory + portability assessment only — absorption decisions wait for CC-211 / CC-232 / CC-234 / CC-237 impl briefs. No direct dep target identified. | ops/token | 2026-05-21 | — | P3 | spike |
...
| CC-060 | ✅ closed 2026-05-23 | Extract codex-dispatch hardcoded params (timeout + model alias SoT) behind config layer; adds `~/.pm-dispatch/config` user-config precedent | arch/config | 2026-05-18 | pr:#131 | P2 | design |
...
## CC-209 — context-pack source evaluation（someday — partial spike done）
...
**Broadened 2026-05-24**: spike scope expanded beyond codegraph to also evaluate **Memori** (https://github.com/MemoriLabs/Memori) and **agentmemory** (https://github.com/rohitg00/agentmemory) — both surfaced as context-pack source candidates. Partial spike result `docs/spikes/CC-209.md` covers Memori + agentmemory + portability lens (user constraint `[[feedback_memory_cli_agnostic]]`) + pilot walkthrough. **codegraph survey still owed** per original scope (install + query + token/accuracy benchmark vs rg/git baseline). Spike is inventory + portability assessment only — absorption decisions wait for CC-211 / CC-232 / CC-234 / CC-237 impl briefs.
