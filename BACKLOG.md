...
**Requirement 2（待做）**：`runtime/bin/doctor.sh` 加一個 Layer-1 檢查——
...
**Requirement 3（待做，與 [[CC-370]] 協調）**：issue 另兩個 minor——`pmctl` 是
...
文件寫明 `function pmctl { bash "<path>" @args }`。
...
**Done-when**：**三個 Requirement 全部完成才轉 done**。Req 1 已達成（三處 parity lint
綠、`test-pmctl-context.sh`＋`test-pmctl-gate.sh`＋`test-pmctl-pm.sh` 綠、pr-gate GO、
pr:#580 merged）。Req 2 = doctor CRLF 檢查落地並有 regression。Req 3 = `pmctl.cmd`
shim 或安裝文件補齊（範圍與 [[CC-370]] 對齊後定案）。
