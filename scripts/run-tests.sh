...
# sign-off. Run scripts/run-all-tests.sh separately for final validation.
#
# Usage:
#   scripts/run-tests.sh [--base <ref>] [--path <repo-relative-path>]...
#                        [--result-file <path>] [--list] [--jobs N] [--suite-timeout N]
#   scripts/run-tests.sh --all [--skip <suite>] [--list] [--jobs N] [--suite-timeout N]
#   scripts/run-tests.sh --verify-full <result-file>
set -euo pipefail
...
map_path() {
  ...
  case "$path" in
    scripts/run-all-tests.sh|scripts/lib/test-suite-runner.sh|scripts/lib/test-harness.sh|scripts/lib/portable.sh|install.sh|uninstall.sh)
...
printf 'run-tests: final sign-off still requires a separate scripts/run-all-tests.sh PASS\n' >&2
