...
g_audit() {
  ...
  # exposure surface. Best-effort like the rest of this function: a failure
  # here must never disturb the allow/deny decision.
  [[ -e "$LOG_FILE" ]] || { : > "$LOG_FILE"; chmod 600 "$LOG_FILE"; } 2>/dev/null || true
  local ts
