...
#
# Extracted from the byte-identical portions of scripts/gate-supervisor.sh /
# scripts/lib/pmctl-gate.sh and scripts/dispatch-supervisor.sh /
# scripts/lib/pmctl-dispatch.sh. Owns ONLY the detach/sentinel mechanics that
...
#
# Sourced by both scripts/gate-supervisor.sh / scripts/dispatch-supervisor.sh
# (after each resolves its own REPO_ROOT) and scripts/lib/pmctl-gate.sh /
