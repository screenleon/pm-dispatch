...
tail -n +2 "$inventory" | cut -f1 | sort > "$declared_paths"
if ! cmp -s "$actual_paths" "$declared_paths"; then
  diff -u "$declared_paths" "$actual_paths" >&2 || true
  fail "scripts/ file set differs from path inventory"
fi
...
collect_refs() {
  ...
  scope="production"
  case "$relative" in
    scripts/test-*.sh|*/test-*.sh|*/fixtures/*) scope="test" ;;
  esac
...
done < <(find "$repo_root/scripts" "$repo_root/adapters" "$repo_root/hosts" \
