...
# Behavior: hk_to_posix_path is a no-op for non-drive-letter (POSIX) paths.
# Steps: call with an absolute POSIX path on the default platform; assert it is
# echoed unchanged with exit 0.
test_to_posix_noop() {
  local name="hk_to_posix_path: POSIX path no-op"
  should_run "$name" || return 0
  local out status=0
  out="$(run_in_framework '{}' hk_to_posix_path '/tmp/x/y')" || status=$?
...
test_to_posix_windows_convert() {
  local name="hk_to_posix_path: Windows drive-letter -> POSIX via cygpath"
  should_run "$name" || return 0
  local stub out status=0
  stub="$(mktemp -d)"
  printf '#!/usr/bin/env bash\np="${!#}"; printf '\''%%s'\'' "/${p#?:/}"\n' > "$stub/cygpath"
  chmod +x "$stub/cygpath"
  out="$(PM_DISPATCH_PLATFORM=windows PATH="$stub:$PATH" run_in_framework '{}' hk_to_posix_path 'C:/Users/foo')" || status=$?
  rm -rf "$stub"
...
test_to_posix_cygpath_unavailable() {
  local name="hk_to_posix_path: drive-letter + no cygpath -> rc 2"
  should_run "$name" || return 0
  ...
  local status=0
  PM_DISPATCH_PLATFORM=windows run_in_framework '{}' hk_to_posix_path 'C:/x' >/dev/null 2>&1 || status=$?
  if [[ "$status" -eq 2 ]]; then
    pass "$name"
  else
...
test_to_posix_cygpath_fail() {
  local name="hk_to_posix_path: drive-letter + cygpath fail -> rc 3"
  should_run "$name" || return 0
  local stub status=0
  stub="$(mktemp -d)"
  printf '#!/usr/bin/env bash\nexit 1\n' > "$stub/cygpath"
  chmod +x "$stub/cygpath"
  PM_DISPATCH_PLATFORM=windows PATH="$stub:$PATH" run_in_framework '{}' hk_to_posix_path 'C:/x' >/dev/null 2>&1 || status=$?
  rm -rf "$stub"
  if [[ "$status" -eq 3 ]]; then
    pass "$name"
  else
