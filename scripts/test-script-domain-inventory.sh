...
fixture_repo() {
  ...
  while IFS=$'\t' read -r current_path _; do
...
test_untracked_script_fails() {
  ...
  expect_fail "$name" "$root" "file set differs"
}

# Behavior: a declared current path that no longer exists is rejected.
# Steps: build a complete fixture, remove one declared file, and assert the file-set diagnostic.
test_missing_script_fails() {
  local name="script-domain-inventory/missing-script-fails" root
  should_run "$name" || return 0
  root="$(fixture_repo)"
  rm -f "$root/scripts/brief-validate.sh"
  expect_fail "$name" "$root" "file set differs"
}
