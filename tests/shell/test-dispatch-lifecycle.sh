...
SUPERVISOR="$REPO_ROOT/runtime/bin/dispatch-supervisor.sh"
...
_part="\$(cd "\$_work" 2>/dev/null && . "$REPO_ROOT/runtime/lib/state-paths.sh" 2>/dev/null && dirname "\$(sw_project_run_dir __probe__ 2>/dev/null)")"
...
case_supervisor_die_restricted_cleanup() {
  ...
  set +e
  bash "$REPO_ROOT/runtime/bin/dispatch-supervisor.sh" --run-spec "$spec_file" \
    >"$supervisor_log" 2>&1
  set -e
