...
# QA category matrix for runtime/lib/pmctl-backlog.sh + cli/pmctl backlog dispatch.
...
# 5. State transitions: N/A - archive mutation semantics live in tests/shell/test-archive-closed-backlog.sh; this file only covers thin forwarding and read-only view behavior
...
# shellcheck source=runtime/lib/pmctl-backlog.sh
. "$REPO_ROOT/runtime/lib/pmctl-backlog.sh"
th_init "$@"
...
setup_archive_repo() {
  ...
  mkdir -p "$repo/scripts" "$repo/ops/backlog"
  cp "$REPO_ROOT/ops/backlog/archive-closed-backlog.sh" "$repo/ops/backlog/archive-closed-backlog.sh"
  chmod +x "$repo/ops/backlog/archive-closed-backlog.sh"
...
setup_archive_failure_repo() {
  ...
  mkdir -p "$repo/ops/backlog"
  cat > "$repo/ops/backlog/archive-closed-backlog.sh" <<'EOF'
#!/usr/bin/env bash
exit 5
EOF
  chmod +x "$repo/ops/backlog/archive-closed-backlog.sh"
