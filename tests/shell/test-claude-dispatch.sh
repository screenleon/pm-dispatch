...
case_state_store_no_direct_run_row_claude() {
  ...
  ln -s "$REPO_ROOT/ops/usage/log-usage.sh" "$home/.claude/scripts/log-usage.sh"
  store="$(mktemp -d)"
...
case_model_alias_malformed_tsv_warns() {
  ...
  set +e
  out="$(bash -s -- "$bad_tsv" "$DISPATCH" "$REPO_ROOT/runtime/lib/model-aliases.sh" 2>&1 <<'RESOLVER_TEST'
set -euo pipefail
