...
case_state_store_init_windows_acl_unsafe_rejected() {
  # issue #592: POSIX chmod/stat mode bits are inert on native Windows Git
  # Bash, so _sw_ensure_store_root_safe must ask the real OS via PowerShell's
  # Get-Acl instead when detect_platform reports windows. Simulate an ACL
  # that grants a non-owner principal (Everyone) write access and assert the
  # store root is rejected, same contract as the POSIX group/world-writable
  # case above.
  #
  # Steps:
  #   1. Create a store root (its real POSIX mode is irrelevant here).
  #   2. Force detect_platform=windows and stub cygpath/powershell.exe so the
  #      simulated Get-Acl reports "UNSAFE".
  #   3. Assert state_store_init fails loudly before writing VERSION.
  local name="state_store_init: Windows ACL write-grant rejected (POSIX mode bits are inert there)"
  should_run "$name" || return 0
  local store stubs rc=0 stderr_out
  store="$tmp_root/root-windows-acl-unsafe"
  mkdir -p "$store"
  stubs="$tmp_root/windows-acl-unsafe-stubs"
  mkdir -p "$stubs"
  ...
  stderr_out="$(PATH="$stubs:$PATH" PM_DISPATCH_PLATFORM=windows \
    PM_DISPATCH_STATE_ROOT="$store" _SW_ALLOW_GLOBAL_PARTITION=1 state_store_init 2>&1 >/dev/null)" || rc=$?
  if [[ "$rc" -ne 0 && "$stderr_out" == *"ACL grants write to a non-owner principal"* && ! -e "$store/VERSION" ]]; then
    pass "$name"
  else
    fail "$name" "rc=$rc stderr=${stderr_out:-empty}"
  fi
...
case_state_store_init_windows_acl_safe_accepted() {
  # Counterpart: a store root whose simulated Get-Acl reports "SAFE" (no
  # non-owner write grant) must be accepted, and — critically — the STORE
  # ROOT itself must NOT go through the POSIX chmod/mode-bit path on Windows
  # (proving the branch is platform-gated, not merely additive). Chmod is
  # still called elsewhere in state_store_init for the per-project directory
  # (out of this guard's scope, best-effort, `|| true`), so the assertion
  # targets the store-root path specifically rather than "chmod never runs".
  local name="state_store_init: Windows ACL safe root accepted, POSIX chmod path skipped for the store root"
  should_run "$name" || return 0
  local store stubs rc=0 stderr_out chmod_log="$tmp_root/windows-acl-safe-chmod-log"
  store="$tmp_root/root-windows-acl-safe"
  mkdir -p "$store"
  stubs="$tmp_root/windows-acl-safe-stubs"
  mkdir -p "$stubs"
  # shellcheck disable=SC2016 # $1/$2 expand when the generated cygpath stub runs.
  printf '#!/usr/bin/env bash\n[[ "$1" == "-w" ]] || exit 1\nprintf "%%s\\n" "$2"\n' > "$stubs/cygpath"
  printf '#!/usr/bin/env bash\nprintf "SAFE\\n"\n' > "$stubs/powershell.exe"
  ...
  stderr_out="$({
    ...
    PATH="$stubs:$PATH" PM_DISPATCH_PLATFORM=windows \
      PM_DISPATCH_STATE_ROOT="$store" _SW_ALLOW_GLOBAL_PARTITION=1 state_store_init
  } 2>&1 >/dev/null)" || rc=$?
...
case_state_store_init_windows_acl_unsafe_escape_hatch() {
  # The existing PM_DISPATCH_ALLOW_UNSAFE_STATE_ROOT escape hatch must cover
  # the Windows ACL rejection path too, same as it covers the POSIX one.
  local name="state_store_init: unsafe-root escape hatch covers a Windows ACL write-grant"
  should_run "$name" || return 0
  local store stubs rc=0 stderr_out
  store="$tmp_root/root-windows-acl-unsafe-allowed"
  mkdir -p "$store"
  stubs="$tmp_root/windows-acl-unsafe-allowed-stubs"
  mkdir -p "$stubs"
  # shellcheck disable=SC2016 # $1/$2 expand when the generated cygpath stub runs.
  printf '#!/usr/bin/env bash\n[[ "$1" == "-w" ]] || exit 1\nprintf "%%s\\n" "$2"\n' > "$stubs/cygpath"
  printf '#!/usr/bin/env bash\nprintf "UNSAFE\\n"\n' > "$stubs/powershell.exe"
...
case_state_store_init_windows_acl_tool_missing_fails_open() {
  ...
  mkdir -p "$store"
  stubs="$tmp_root/windows-acl-unverifiable-stubs"
  mkdir -p "$stubs"
  # shellcheck disable=SC2016 # $1/$2 expand when the generated cygpath stub runs.
  printf '#!/usr/bin/env bash\n[[ "$1" == "-w" ]] || exit 1\nprintf "%%s\\n" "$2"\n' > "$stubs/cygpath"
  printf '#!/usr/bin/env bash\nexit 1\n' > "$stubs/powershell.exe"
...
case_project_key_shasum_fallback() {
  ...
  result="$(
    FAKE_SHA1SUM_MISSING=1 PATH="$fake_bin:$PATH" _SW_REPO_ROOT="$tmp_git" \
      bash -c "source '$REPO_ROOT/runtime/lib/state-writer.sh' 2>/dev/null
               _sw_project_key"
  )" || true
...
case_project_key_no_sha1sum() {
  ...
  result="$(
    FAKE_SHA1_MISSING=1 _SW_REPO_ROOT="$tmp_root" \
      bash -c "source '$REPO_ROOT/runtime/lib/state-writer.sh' 2>/dev/null; _sw_project_key"
  )" || true
  rm -rf "$tmp_root"
