...
case_happy_path() {
  ...
  run_skill_refine "$repo" testskill "$REPO_ROOT/tests/fixtures/skill-refine" "$tmp_root/no-home" "$out" "$err"
  status=$RUN_STATUS

  assert_exit "$name" "$status" 0
  assert_file_contains "$name" "$out" "# Skill refine signals: testskill"
  assert_file_contains "$name" "$out" "Found 2 candidate(s). Scanned $REPO_ROOT/tests/fixtures/skill-refine at "
  ...
  assert_file_contains "$name" "$out" "- file: $REPO_ROOT/tests/fixtures/skill-refine/feedback_alpha.md"
...
case_missing_command_file() {
  ...
  run_skill_refine "$repo" missing "$REPO_ROOT/tests/fixtures/skill-refine" "$tmp_root/no-home" "$out" "$err"
  status=$RUN_STATUS
...
case_no_args_exits_2_with_usage() {
  ...
  err="$tmp_root/$name.err"
  set +e
  bash "$repo/tools/skills/skill-refine.sh" > "$out" 2> "$err"
  local status=$?
  set -e
  assert_exit "$name" "$status" 2
...
case_multi_args_exits_2_with_usage() {
  ...
  err="$tmp_root/$name.err"
  set +e
  bash "$repo/tools/skills/skill-refine.sh" foo bar > "$out" 2> "$err"
  local status=$?
  set -e
  assert_exit "$name" "$status" 2
  assert_file_contains "$name" "$err" "Usage:"
