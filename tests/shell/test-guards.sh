...
memory_usage_commit_concurrent_no_lost_updates() {
  ...
  # below, rather than using process-heavy shell locks as its proxy.
...
memory_usage_commit_contention_matrix() {
  ...
  if [[ "$status" -eq 0 ]] \
    && [[ "$(grep -Ec '^backend=(flock|mkdir) round=[1-2] count=8 started=8 acquired=8 finished=8 exit0=8 ids-ok=1 lockdir=absent$' <<< "$report")" -eq 4 ]]; then
    pass "$name"
  else
...
memory_usage_sqlite_concurrent_atomic_updates() {
  # Primary-store stress test: direct writers begin together and update one key
  # without serialize_with_lock. SQLite must retain every increment in WAL mode.
...
memory_usage_sqlite_imports_legacy_once() {
  # Upgrade contract: the first SQLite transaction imports a valid TSV store;
  # later transactions must never replay that snapshot.
...
memory_usage_sqlite_quotes_card_relpath() {
  # SQL boundary: a markdown relpath containing an apostrophe remains a plain
  # key and cannot alter the transaction assembled for SQLite.
  ...
  report="$(
    ...
    sqlite3 -separator ' ' "$db" \
      "SELECT card_relpath, access_count FROM card_usage;"
    rm -rf "$d"
  )" || status=$?
