...
#
# Usage:
#   tests/shell/test-usage-tracker.sh           # print PASS per case
#   VERBOSE=1 tests/shell/test-usage-tracker.sh # include diagnostics on failure

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
LOG_SCRIPT="$REPO_ROOT/ops/usage/log-usage.sh"
VIEW_SCRIPT="$REPO_ROOT/ops/usage/token-usage.sh"
