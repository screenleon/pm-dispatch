#!/usr/bin/env bash
# Regression tests for tools/lint/lint-test-docstrings.sh.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
LINTER="$REPO_ROOT/tools/lint/lint-test-docstrings.sh"
...
test_self_cannot_be_allowlisted() {
  ...
  should_run "$name" || return 0
  local output status
  output="$(cd "$REPO_ROOT" && bash "$LINTER" --allow tests/shell/test-lint-test-docstrings.sh 2>&1)"; status=$?
  if [[ $status -ne 0 ]]; then
    pass "$name"
  else
