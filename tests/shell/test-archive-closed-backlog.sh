#!/usr/bin/env bash
# Regression tests for ops/backlog/archive-closed-backlog.sh.
#
...
ARCHIVER="$REPO_ROOT/ops/backlog/archive-closed-backlog.sh"
...
setup_repo() {
  ...
  cp "$ARCHIVER" "$repo/ops/backlog/archive-closed-backlog.sh"
...
run_archiver() {
  local repo="$1"
  shift
  (cd "$repo" && bash "$repo/ops/backlog/archive-closed-backlog.sh" "$@")
}
...
case_unknown_flag() {
  ...
  local output rc=0
  set +e
  output=$(bash "$repo/ops/backlog/archive-closed-backlog.sh" --unsupported-flag 2>&1)
  rc=$?
  set -e
