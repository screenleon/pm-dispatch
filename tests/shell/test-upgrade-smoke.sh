#!/usr/bin/env bash
# Focused CLI and evidence-contract tests for the CC-501 upgrade smoke.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
SMOKE="$REPO_ROOT/ops/release/upgrade-smoke-v0.8-v0.9.sh"

# shellcheck source=tests/lib/test-harness.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/../lib/test-harness.sh"
th_init "test-upgrade-smoke" "$@"
...
# Behavior: the release blocker records every required CC-501 evidence class.
