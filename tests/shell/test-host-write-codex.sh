...
test_host_write_dispatches_codex_from_manifest() {
  ...
  should_run "$name" || return 0
  local codex_home="$tmp_root/hwd-codex/.codex" claude_config="$tmp_root/hwd-codex/.claude" xdg_config="$tmp_root/hwd-codex/xdg"
  ...
  CLAUDE_CONFIG_DIR="$claude_config" CODEX_HOME="$codex_home" XDG_CONFIG_HOME="$xdg_config" \
    host_write_uninstall_all "$REPO_ROOT" 0 >/dev/null 2>&1
...
test_host_write_dispatches_opencode_stage3() {
  ...
  should_run "$name" || return 0
  local xdg="$tmp_root/hwd-opencode" codex="$tmp_root/hwd-opencode-codex" claude_config="$tmp_root/hwd-opencode-claude"
  ...
  CLAUDE_CONFIG_DIR="$claude_config" XDG_CONFIG_HOME="$xdg" CODEX_HOME="$codex" \
    host_write_uninstall_all "$REPO_ROOT" 0 >/dev/null 2>&1
