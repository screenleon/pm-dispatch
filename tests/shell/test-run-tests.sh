...
# Behavior: runtime/lib/gate-policy.sh selects its own unit suite (plus the
# pr-gate shards, since pr-gate.sh sources it); a pr-gate.sh change also pulls
# in test-gate-policy.
case_gate_policy_lib_maps_its_suite() {
  local name=gate-policy-lib-maps-its-suite repo out status=0 args
  args="$TMP_ROOT/$name.args"
  repo="$(make_fixture "$name")"
  out=$(RUN_TESTS_ARGS_LOG="$args" "$repo/tests/bin/run-tests.sh" \
    --path runtime/lib/gate-policy.sh --list 2>&1) || status=$?
  if [[ "$status" -ne 0 || "$out" != *"test-gate-policy"* \
        || "$out" != *"test-pr-gate-shard-1"* || "$out" == *"coverage gaps"* ]]; then
    fail "$name" "gate-policy.sh path: status=$status out=$out"; return
  fi
  status=0
  out=$(RUN_TESTS_ARGS_LOG="$args" "$repo/tests/bin/run-tests.sh" \
    --path runtime/bin/pr-gate.sh --list 2>&1) || status=$?
  if [[ "$status" -eq 0 && "$out" == *"test-gate-policy"* && "$out" != *"coverage gaps"* ]]; then
    pass "$name"
  else
    fail "$name" "pr-gate.sh path: status=$status out=$out"
  fi
