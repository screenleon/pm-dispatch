...
DOCTOR="$REPO_ROOT/runtime/bin/doctor.sh"
  ...
  cat > "$home_dir/.claude/settings.json" <<EOF
{
  "hooks": {
    ...
    "Stop": [
      ...
      {"hooks": [{"type": "command", "command": "/fake/old-repo/runtime/hooks/guard-session-summary.sh --host claude"}]}
    ],
    "UserPromptSubmit": [
  ...
  cat > "$home_dir/.claude/settings.json" <<EOF
{
  "hooks": {
    "PreToolUse": [
      {"matcher": "Edit|Write", "hooks": [{"type": "command", "command": "${sibling}/runtime/hooks/guard-pm-write.sh"}]},
    ...
    "Stop": [
      ...
      {"hooks": [{"type": "command", "command": "${sibling}/runtime/hooks/guard-session-summary.sh --host claude"}]}
    ],
    "UserPromptSubmit": [
...
case_doctor_scripts_not_executable_fail() {
  ...
  # Create a writable copy of the owned executable domains to safely toggle perms.
...
case_doctor_context_hook_timeout_too_low_fails() {
  # Verifies doctor catches the exact live regression where Claude's default
  # 30-second handler timeout is shorter than the context hook's own budget.
  # Steps:
  #   1. Write an otherwise valid minimal install with timeout=30
  #   2. Run doctor in minimal profile
  #   3. Assert the hooks check fails with the timeout-envelope diagnosis
  local name="doctor-context-hook-timeout-too-low-fails"
  should_run "$name" || return 0
  if ! command -v jq >/dev/null 2>&1; then
    pass "$name (jq not available - skip)"
    return
  ...
  create_memory_dir_for_pwd "$home"
  write_manifest "$home"
  path="$(make_stub_bin "$tmp_root/bin-context-timeout-low" claude)"

  out="$(HOME="$home" CLAUDE_CONFIG_DIR="$home/.claude" PATH="$path" \
    bash "$DOCTOR" --no-color --repo "$REPO_ROOT" --profile minimal 2>&1)" || status=$?
  if [[ "$status" -eq 1 && "$out" == *"timeout missing or below 150s"* ]]; then
    pass "$name"
  else
    fail "$name" "expected timeout-envelope failure; status=$status out=$out"
  fi
}
...
case_doctor_codex_retired_hook_target_fails() {
  # A configured hook from this checkout must not be considered healthy after
  # its compatibility path is retired.
  local name="doctor-codex-retired-hook-target-fails"
  should_run "$name" || return 0
  local home="$tmp_root/home-codex-retired-target" out status=0 path
  write_full_settings "$home"
  create_memory_dir_for_pwd "$home"
  write_manifest "$home"
  CODEX_HOME="$home/.codex" bash "$REPO_ROOT/hosts/codex/bin/install.sh" \
    --repo-root "$REPO_ROOT" >/dev/null 2>&1
  jq --arg old "$REPO_ROOT/scripts/guard-inject-memory.sh" '
    .hooks.UserPromptSubmit[0].hooks[0].command = $old
  ' "$home/.codex/hooks.json" > "$home/.codex/hooks.json.tmp"
  mv "$home/.codex/hooks.json.tmp" "$home/.codex/hooks.json"
  path="$(make_stub_bin "$tmp_root/bin-codex-retired-target" claude codex)"
  ...
  if [[ "$status" -eq 1 ]] && printf '%s\n' "$out" | jq -e '
    ...
    fail "$name" "expected retired Codex hook target to fail; status=$status out=$out"
  fi
}

case_doctor_claude_retired_hook_target_fails() {
  # Basename-only presence is insufficient: a configured managed path must
  # still exist and be executable.
  local name="doctor-claude-retired-hook-target-fails"
  should_run "$name" || return 0
  local home="$tmp_root/home-claude-retired-target" out status=0 path
  write_full_settings "$home"
  create_memory_dir_for_pwd "$home"
  write_manifest "$home"
  jq --arg old "$REPO_ROOT/scripts/guard-inject-memory.sh" '
  ...
  ' "$home/.claude/settings.json" > "$home/.claude/settings.json.tmp"
  mv "$home/.claude/settings.json.tmp" "$home/.claude/settings.json"
  path="$(make_stub_bin "$tmp_root/bin-claude-retired-target" claude codex)"
  ...
  if [[ "$status" -eq 1 ]] && printf '%s\n' "$out" | jq -e '
    ...
    fail "$name" "expected retired Claude hook target to fail; status=$status out=$out"
  fi
}
...
case_doctor_repo_trusted_linter() {
  ...
  # tools/lint/lint-frontmatter.sh, preventing arbitrary code execution via --repo.
  #
  # Steps:
  ...
  mkdir -p "$fake_repo/scripts" "$fake_repo/agents" "$fake_repo/commands" "$fake_repo/tools/lint"
  cat > "$fake_repo/tools/lint/lint-frontmatter.sh" <<EOLINT
#!/usr/bin/env bash
touch "$sentinel"
exit 0
EOLINT
  chmod +x "$fake_repo/tools/lint/lint-frontmatter.sh"
