...
#
# Usage:
#   tests/shell/test-usage-weekly.sh           # print PASS per case
#   VERBOSE=1 tests/shell/test-usage-weekly.sh # include command diagnostics on failure

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
USAGE="$REPO_ROOT/ops/usage/usage-weekly.sh"
