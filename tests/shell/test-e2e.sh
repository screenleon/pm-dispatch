...
if ! command -v "$ADAPTER" >/dev/null 2>&1; then
  record "$ADAPTER on PATH" SKIP "not found — E2E requires an authenticated live adapter; run manually (./tests/shell/test-e2e.sh --adapter $ADAPTER)"
...
if [[ "$dispatch_rc" -ne 0 ]]; then
  record "real dispatch" SKIP "dispatch exited $dispatch_rc — adapter may need authentication; run manually: ./tests/shell/test-e2e.sh --adapter $ADAPTER"
  REQUIRED_SKIPPED=$((REQUIRED_SKIPPED + 1))
