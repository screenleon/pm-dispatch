...
FAKEOF
  ...
  ln -s "$REPO_ROOT/ops/usage/log-usage.sh" "$_home11/.claude/scripts/log-usage.sh"
...
FAKEOF
  ...
  ln -s "$REPO_ROOT/ops/usage/log-usage.sh" "$_home12/.claude/scripts/log-usage.sh"
...
case_alias_source_installed_helper_fallback() {
  ...
  cp "$REPO_ROOT/runtime/lib/model-aliases.sh" "$_script_dir/lib/"
  cp "$REPO_ROOT/runtime/lib/reasoning-effort.sh" "$_script_dir/lib/"
  cp "$REPO_ROOT/runtime/lib/timeout-resolve.sh" "$_script_dir/lib/"
  cp "$REPO_ROOT/runtime/lib/dispatch-common.sh" "$_script_dir/lib/"
...
FAKEOF
  ...
  ln -s "$REPO_ROOT/ops/usage/log-usage.sh" "$_home/.claude/scripts/log-usage.sh"
  _store="$(mktemp -d)"
  _work="$(mktemp -d)"; git init -q "$_work"
  _brief="$(mktemp --suffix=.md)"
