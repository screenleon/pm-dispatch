...
# block-extraction approach tests/shell/test-host-manifest.sh and
...
#   host_manifest_expand_root_template <label> <token> <root> <path_template>
...
# variable in shared code. A token repeated later in the path remains literal.
host_manifest_expand_root_template() {
  local label="$1" token="$2" root="$3" path="$4"
  case "$path" in
    "$token"|"$token"/*)
      printf '%s%s\n' "$root" "${path#"$token"}"
      ;;
    *)
      printf '%s path resolver: unsupported manifest path template: %s\n' "$label" "$path" >&2
      return 2
      ;;
  esac
}
