...
#   isolation_map_ref, dispatch_entrypoint, dispatch_contract, runner_kind,
#   generated_files
...
# needs_bash_guard derive from it (runtime/lib/runner-kind.sh). Default to
...
EOF

  {
    ...
    printf '# uses sandbox: + config_overrides: format (consumed by adapters/codex/dispatch.sh).\n'
...
# The executor entrypoint pmctl invokes (resolved from dispatch_entrypoint in
# adapter.yaml). Scaffolded as a STUB so a freshly
...
# and dispatch_entrypoint: the dispatch allowlist is derived from validated
# manifests, so no edit to
# runtime/lib/executor-router.sh is required.
...
This adapter has two scripts. The runtime executable is selected only by
\`adapter.yaml:dispatch_entrypoint\`:
...
# manifests, so a valid runner_kind + dispatch_entrypoint registers it — no
# edit to runtime/lib/executor-router.sh is needed.
