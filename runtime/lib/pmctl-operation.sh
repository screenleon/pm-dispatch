...
_pmctl_operation_reconcile_inner() {
  ...
  if [[ -f "$op_dir/children.jsonl" ]]; then
    while IFS= read -r line; do
      ...
      pm_identifier_run_is_valid "$run_id" && [[ "$child_dir" == /* ]] || { missing=$((missing + 1)); continue; }
      children=$((children + 1))
...
pmctl_operation_cancel() {
  ...
  pm_identifier_operation_is_valid "$operation_id" && [[ -n "$work_dir" && -d "$work_dir" ]] || { _pmctl_operation_usage; return 2; }
  _pmctl_operation_load_writer "$repo_root" || return 2
  ...
  if [[ -f "$op_dir/children.jsonl" ]]; then
    while IFS= read -r line; do
      ...
      pm_identifier_run_is_valid "$run_id" && [[ "$child_dir" == /* && "$seen" != *" $run_id "* ]] || continue
      seen+="$run_id "
...
pmctl_operation_reconcile() {
  ...
  pm_identifier_operation_is_valid "$operation_id" && [[ -n "$work_dir" && -d "$work_dir" ]] || { _pmctl_operation_reconcile_usage; return 2; }
  _pmctl_operation_load_writer "$repo_root" || return 2
  local rc=0
  _pmctl_operation_with_record_lock "$repo_root" "$work_dir" "$operation_id" \
