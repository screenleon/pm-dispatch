...
# CC-532: option defaults, flag parsing, and --cd identity checks belong to
# gate-options.sh. The entrypoint only invokes the canonical parser.
# Names are declared here so this file's ShellCheck analysis can see them;
# the module overwrites every value.
WORK_DIR=""
TIMEOUT=""
gate_options_init
gate_options_parse "$@"
...
# ── Load gate overrides ───────────────────────────────────────────────────────
# The loader lives in gate-reviewer-contract.sh. Snapshot unlink stays on this
# entrypoint's EXIT trap so a mid-load abort still removes the private copy.
