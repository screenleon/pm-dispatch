...
| CC-239 | **reuse-scan + context pack 組裝**（spine 的 user-visible 終點）：PM briefing 時查 prior art，輸出 `reuse_candidates`（helper / test pattern + why_relevant）並組裝 `pmctl context pack` 統一輸出（含 summary/tags）；brief 吸收。repo-index 的第一個 consumer，直接消化 reuse-debt | ✅ pr:#TBD |
