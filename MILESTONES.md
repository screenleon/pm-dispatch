...
| CC-425 | gate 解除 PR 綁定：`pr-gate.sh --head <ref>` 新增，diff 固定 base..head ref 對，不涉 PR/working tree；盤點發現 result 路徑 PR# key 問題已在 CC-423 detached lifecycle 重構中解決（改用 gate_id），`--base` 也已支援無 PR 場景，故實際範圍小於原評估 | ✅ done pr:#355 |
