...
| CC-356 | **接線：context pack / reuse-scan 進 dispatch 流程 + 使用可觀測**（新增 2026-06-10）。pack/reuse-scan（#256）操作面零 caller——中立 docs 契約 + PM/skill 指標要求 brief 前先查 prior art；reuse_candidates 設 hit 上限防 brief 噪音；query/reuse-scan emit event 使使用次數可由 `pmctl trace` 量測。亦為 CC-346 的 resume-trigger 證據來源 | P2 |
