...
| CC-399 | **(7c-2b)** `pmctl dispatch run --lifecycle detached` now launches the supervisor via `setsid`/`nohup`, returns `run_id` immediately, writes advisory supervisor PID/log sidecars, and `pmctl dispatch wait <run_id> --cd <dir>` reattaches through the durable `.dispatch-results/<run_id>.md` record only. `pmctl inbox`/notify channel remain out of this thin slice. | ✅ pr:#298 |
