...
# top of tests/lib/test-harness.sh) on an explicit allowlist of
# tests/shell/test-*.sh files that have already been converted to it.
#
...
if [ "${#custom_allow[@]}" -gt 0 ]; then
  ...
  # tests/shell/test-lint-test-docstrings.sh is deliberately NOT in this list:
  ...
  # test_self_cannot_be_allowlisted in tests/shell/test-lint-test-docstrings.sh
  # for the regression test that pins this known limitation.
  ALLOWLIST=(
    tests/shell/test-guard-framework.sh
    tests/shell/test-migrate-routing-log.sh
    tests/shell/test-migrate-routing-to-events.sh
    tests/shell/test-pr-gate.sh
  )
fi
