...
3. **Context retrieval reflex** — Mandatory order: run `pmctl context query --domain knowledge <term>` before Read/Grep/full-file opens on knowledge docs; only if it returns no hits, fall back to targeted Read/Grep. Before writing `files:` / `context:` in a brief, run `pmctl context reuse-scan <working_dir> "<task description>"` (pass the target repo root, not pm-dispatch) to surface prior-art anchors; spec at `docs/context-retrieval.md`.
