...
**Dispatch model selection**: the default is the `default` alias, which resolves to `gpt-5.5` (defined in `share/model-aliases.tsv`; pinned by pm-dispatch independent of the host's interactive `~/.codex/config.toml`, with `gpt-5.4` as the config-set fallback). Omitting `--model` therefore dispatches on `gpt-5.5` — do NOT assume the ambient codex config supplies the model. Use `--model codex-spark` only when all three criteria are met: (a) expected diff < 50 lines, (b) changes confined to ≤ 2 adjacent files with no cross-module dependencies, (c) no new interfaces, abstractions, or hooks introduced. `codex-spark` draws from a separate, independent usage pool and has a lower context ceiling — misrouting a large task degrades output quality without a loud failure signal, so reserve it for known-small, single-location changes. Alias-to-wire-format mapping is documented in `docs/dispatch-brief.md` §Model aliases so PM-facing short names remain stable while dispatch uses wire format internally.
