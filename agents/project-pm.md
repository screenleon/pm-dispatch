...
**`files:` block is a sandbox allowlist, not a must-read list** (per `docs/dispatch-brief.md` §`files:` block semantics): a `read:` entry means the executor MAY open that file, NOT that it MUST pre-load it. For survey-style briefs (spikes, audits) whose `files:` block lists > 4 reads or > 50KB total, add a `context:` instruction telling the executor to read on demand via `grep -n` / `sed -n` / section-targeted commands — never full-file Read on `BACKLOG.md`, `agents/*.md`, or large synthesis docs. Calibration: CC-229 substrate spike v1/v2 hit codex `context_length_exceeded` with eager-load `files:` blocks; v3 succeeded after a lazy-read `context:` instruction.

**Multi-file brief discipline** (when the brief touches > 4 files OR embeds > 50 lines of verbatim content the executor must reproduce byte-identically — see `[[feedback_codex_brief_discipline]]` for retro): include all three of these to prevent the apply_patch debug-loop hang pattern observed on CC-247/248 (#142) and CC-250 (#144).
...
2. **Verbatim-as-attached-file**: instead of embedding > 50 lines of literal text (override-policy paragraphs, BACKLOG rows, brief-template fragments) inside the brief's `context:` block, write each block to `/tmp/<task>-content/<name>.md` BEFORE dispatch and reference the path from the brief (`"copy /tmp/cc250-content/override-policy-critic.md verbatim into agents/critic.md; do NOT paraphrase"`). Eliminates codex's hallucinate-when-retyping failure mode (CC-250 stderr showed `pass/fail print-format` → `print-format` — the "pass/fail" was silently dropped during re-typing).
