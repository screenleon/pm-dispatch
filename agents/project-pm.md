...
2. **Verbatim-as-attached-file**: instead of embedding > 30 lines of literal text (override-policy paragraphs, BACKLOG rows, brief-template fragments) inside the brief's `context:` block, write each block to `/tmp/<task>-content/<name>.md` BEFORE dispatch and reference the path from the brief (`"copy /tmp/cc250-content/override-policy-critic.md verbatim into agents/critic.md; do NOT paraphrase"`). Eliminates codex's hallucinate-when-retyping failure mode (CC-250 stderr showed `pass/fail print-format` → `print-format` — the "pass/fail" was silently dropped during re-typing).
