...
> **Lifecycle-leak warning:** This agent is now a 5-condition fallback, not the primary `/pm` execution path. The primary route is main-thread `Bash(pmctl dispatch run --adapter codex, run_in_background:true)` from a `dispatch_handover_v1` block. Phase 3 post-verification is `scripts/dispatch-post-verify.sh`, which reads `.agent-trace/latest.last` written by the adapter. Use this agent only for the fallback allowlist in §When NOT to use this agent, with `docs/dispatch-brief.md` §Fallback as the canonical policy; see `[[feedback_codex_dispatch_lifecycle_leak]]`.
